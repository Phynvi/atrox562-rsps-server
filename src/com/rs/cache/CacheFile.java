	...
	public final byte[] getContainerData(int containerId) {
		synchronized (dataFile) {
			try {
				...
				int containerSize = (cacheFileBuffer[2] & 0xff)
						+ (((0xff & cacheFileBuffer[0]) << 16) + (cacheFileBuffer[1] << 8 & 0xff00));
				int sector = ((cacheFileBuffer[3] & 0xff) << 16)
						- (-(0xff00 & cacheFileBuffer[4] << 8) - (cacheFileBuffer[5] & 0xff));
				...
				while (containerSize > dataReadCount) {
					...
					int nextSector = (cacheFileBuffer[6] & 0xff) + (0xff00 & cacheFileBuffer[5] << 8)
