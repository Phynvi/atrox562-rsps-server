...
public class NPCExamines {
    ...
    public static void loadExamines() throws IOException {
        if (new File(PACKED).exists())
            ...
            loadUnpacked();
        System.out.println("Loaded " + EXAMINES.size() + " npc examines.");
    }
 
    /**
     * Shows the examine message for the parameter npcId.
     * @param npcId The npc to find the examine of.
    ...
    private static void loadPacked() {
        try {
            ...
            while (buffer.hasRemaining())
                EXAMINES.put(buffer.getShort() & 0xffff,
                        readString(buffer));
            channel.close();
            in.close();
        } catch (IOException e) {
