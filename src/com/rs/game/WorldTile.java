...
public class WorldTile implements Serializable {
	...
	private int getLocalX(WorldTile tile) {
		return getLocalX(tile, 0);
	}

	private int getLocalY(WorldTile tile) {
	...
	public int getDistance(WorldTile from) {
		int distanceX = Math.abs(from.getX() - this.getX());
		int distanceY = Math.abs(from.getY() - this.getY());
