			...
			for (int dynX = 0; dynX < 8; dynX++) {
				for (int dynY = 0; dynY < 8; dynY++) {
					...
					int landArchiveId = Cache.getCacheFileManagers()[5]
							.getContainerId("l" + (mapID >> 8) + "_"
									+ (mapID & 0xFF));
					int mapArchiveId = Cache.getCacheFileManagers()[5]
							.getContainerId("m" + (mapID >> 8) + "_"
									+ (mapID & 0xFF));
					byte[] mapContainerData = mapArchiveId == -1 ? null
							: Cache.getCacheFileManagers()[5].getFileData(mapArchiveId,
									0);
					byte[] landContainerData = landArchiveId == -1 ? null
							: Cache.getCacheFileManagers()[5].getFileData(
									landArchiveId, 0,
									MapContainersXteas.getMapContainerXteas(mapID));
							...
							while ((incr2 = landStream.readUnsignedSmart()) != 0) {
								...
								if (realZ == renderChunkZ
									...
									int[] coords = translate(x & 0x7, y & 0x7,
											rotation, definition.getSizeX(),
											definition.getSizeY(), rot);
									spawnObject(
											new WorldObject(
													objectId,
													type,
					...
					if (Settings.DEBUG && landContainerData == null
							&& landArchiveId != -1
							&& MapContainersXteas.getMapContainerXteas(mapID) != null)
