...
public abstract class Entity extends WorldTile {
	...
	public void processReceivedHits() {
		...
		Hit hit;
		int count = 0;
		while ((hit = receivedHits.poll()) != null && count++ < 1)
			processHit(hit);
	}
	...
	public void removeHitpoints(Hit hit) {
		if (isDead())
			return;
		/*
		 * if (hit.getLook() == HitLook.HEALED_DAMAGE) { heal(hit.getDamage());
		 * return; }
		 */
		...
		if (hitpoints <= 0)
			...
			if (player.getEquipment().getRingId() == 2550) {
				if (hit.getSource() != null && hit.getSource() != player && hit.getDamage() > 5)
					hit.getSource().applyHit(new Hit(player, (int) (hit.getDamage() * 0.1), HitLook.DESEASE_DAMAGE));
			}
			if (player.getPrayer().hasPrayersOn()) {
				if ((hitpoints < player.getMaxHitpoints() * 0.1) && player.getPrayer().usingPrayer(0, 23)) {
					...
					setHitpoints((int) (hitpoints + player.getSkills().getLevelForXp(Skills.PRAYER) * 2.5) / 10);
	...
	private void processReceivedDamage() {
		for (Entity source : receivedDamage.keySet()) {
			Integer damage = receivedDamage.get(source);
			if (damage == null || source.hasFinished()) {
				receivedDamage.remove(source);
...
*/
	...
	public boolean needMasksUpdate() {
		return nextFaceEntity != -2 || nextAnimation != null
				|| nextGraphics != null || nextHit1!= null
				|| nextHit2 != null 
	...
	//Player player = (Player) this;
	//player.starter == 0
	public void setHitpoints1(int hitpoints) {
		this.hitpoints = hitpoints;
	}
...
/*
	public void setNextGraphics(Graphics nextGraphics) {
		if (nextGraphics == null) {
			...
			if (nextGraphics.equals(nextGraphics))
				return;
			if (nextGraphics == null)
				nextGraphics = nextGraphics;
		}
	}
