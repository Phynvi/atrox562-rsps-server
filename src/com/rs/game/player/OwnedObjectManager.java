...
public class OwnedObjectManager {
	...
	public static boolean convertIntoObject(WorldObject object, WorldObject toObject) {
		for (OwnedObjectManager manager : ownedObjects.values()) {
			...
			/* && manager.getCurrentObject().getId() == object.getId() */) {// Temporarily
																			// hiding
																			// due
																			// to
																			// box
	...
	public static Player getOwner(WorldObject object) {
		ownedObjects.forEach((k, v) -> {
			if (v.getCurrentObject().equals(object)) {
				p = v.player;
				return;
			}
	...
	public OwnedObjectManager(Player player, WorldObject[] objects,
			long cycleTime) {
