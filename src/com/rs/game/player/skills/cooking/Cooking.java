...
public class Cooking extends Action {
    ...
    public enum Cookables {
        ...
        RAW_POTATO(new Item(1942, 1), 7, 25, 15, new Item(6699, 1), new Item(6701, 1), false, false),
        ...
        Cookables(Item raw, int lvl, int burningLvl, int exp, Item burnt, Item total, boolean spitRoast,
                          boolean fireOnly) {
            this.raw = raw;
            this.lvl = lvl;
    ...
    public boolean start(Player player) {
        if ((this.cook = Cookables.forId(item.getId())) == null)
            return false;
        if (cook.isFireOnly() && !isFire(object))
            ...
            player.sendMessage("You need a cooking level of " + cook.getLvl() + " to cook this food.");
            return false;
        }
    ...
    public static boolean isFire(WorldObject object) {
        switch (object.getId()) {
            case 5249:
            case 11404:
            case 11405:
    ...
    public static boolean canCook(WorldObject object, Item item, Player player) {
        switch (object.getId()) {
            case 5249:
            case 11404:
            case 11405:
            ...
            case 14919:
                ...
                player.sendMessage("You can't cook that on a " + (Cooking.isFire(object) ? "fire" : "range") + ".");
                return false;
            default:
                return false;
        }
    ...
    public boolean process(Player player) {
        ...
        if (player.getSkills().getLevel(Skills.COOKING) < cook.getLvl()) {
            player.sendMessage("You need a level of " + cook.getLvl() + " to cook this.");
            return false;
        }
        return true;
    }
    ...
    public int processWithDelay(Player player) {
        amount--;
        player.setNextAnimation(isFire(object) ? FIRE_COOKING : RANGE_COOKING);
        if (player.getSkills().getLevel(Skills.COOKING) < cook.getBurningLvl() && isBurned(cook, player)) {
            player.getInventory().deleteItem(item.getId(), 1);
            player.getInventory().addItem(cook.getBurntId().getId(), cook.getBurntId().getAmount());
            player.getPackets().sendGameMessage(
                    "You accidently burnt the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".",
                    true);
        } else {
            player.getInventory().deleteItem(item.getId(), 1);
