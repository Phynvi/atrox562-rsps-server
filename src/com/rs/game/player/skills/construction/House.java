...
import com.rs.game.player.skills.construction.HouseConstants.HObject;
import com.rs.game.player.skills.construction.HouseConstants.Room;
...
public class House implements Serializable {
	...
	private transient boolean loaded, spawnedPet;
	// private transient boolean spawnedPet = false;
	/*
	 * private int storedPet; public int getStoredPet() { return storedPet; }
	 * public void setStoredPet(int storedPet) { this.storedPet = storedPet; }
	 */
	public int storedPets[];
	...
	public static void leaveHouse(Player player) {
		...
		if (controler == null || !(controler instanceof HouseControler)
				|| !((HouseControler) controler).getHouse().getPlayers().contains(player)) {
			player.getPackets().sendGameMessage("You're not in a house.");
			return;
		}
		if (((HouseControler) controler).getHouse() == null)
			return;
	...
	public void build(int slot) {
		...
		final Builds build = (Builds) owner.getTemporaryAttributtes().get("OpenedBuild");
		WorldObject object = (WorldObject) owner.getTemporaryAttributtes().get("OpenedBuildObject");
		...
		if (owner.getSkills().getLevel(Skills.CONSTRUCTION) < piece.getLevel()) {
			owner.getPackets().sendGameMessage("Your level of construction is too low for this build.");
			return;
		}

		if (!owner.getDisplayName().equalsIgnoreCase("Jens")) {
			if (!owner.getInventory().containsItems(piece.getRequirements())) {
				owner.getPackets().sendGameMessage("You dont have the right materials.");
				return;
			}
			if (build.isWater() ? !hasWaterCan()
					: (!owner.getInventory().containsItem(HouseConstants.HAMMER, 1)
							|| !owner.getInventory().containsItem(HouseConstants.SAW, 1))) {
				owner.getPackets()
						.sendGameMessage(build.isWater()
	...
	public void buildDungeonStairs(int slotId, final Builds stair, final RoomReference newRoom) {
		if (owner.getInterfaceManager().containsInterface(396)) {
			for (int i = 0; i < BUILD_INDEXES.length; i++)
	...
	public void climbStaircase(WorldObject object, boolean up, Player player) {
		if (!owner.getHouse().loaded)
			return;
		int roomX = object.getChunkX() - owner.getHouse().boundChuncks[0];
		int roomY = object.getChunkY() - owner.getHouse().boundChuncks[1];
		...
		if (roomTo.getStaircaseSlot() == -1 && ((roomTo.getRoom() != HouseConstants.Room.GARDEN)
				&& (roomTo.getRoom() != HouseConstants.Room.FORMAL_GARDEN))) {
			player.getPackets().sendGameMessage("These stairs do not lead anywhere.");
			return;
		}
					...
					for (int plane = 2; plane >= 1; plane--) {
						if (data[plane][x][y] != null) {
							...
							if (hasRoof) {
								...
								// TODO find best Roof
								data[plane + 1][x][y] = new Object[] { HouseConstants.Roof.ROOF1.getChunkX(),
										HouseConstants.Roof.ROOF1.getChunkY(), rotation, true };
								continue skipY;
							}
						}
					}
		...
		for (int plane = 0; plane < data.length; plane++) {
			for (int x = 0; x < data[plane].length; x++) {
				for (int y = 0; y < data[plane][x].length; y++) {
					if (data[plane][x][y] != null)
						// RegionBuilder
						RegionBuilder.copyChunk((int) data[plane][x][y][0] + (look >= 4 ? 8 : 0),
						...
						RegionBuilder.copyChunk(HouseConstants.BLACK[0], HouseConstants.BLACK[1], 0,
								boundChuncks[0] + x, boundChuncks[1] + y, plane, 0);
					else if (plane == 1)
						RegionBuilder.copyChunk(HouseConstants.LAND[0] + (look >= 4 ? 8 : 0), HouseConstants.LAND[1],
								look & 0x3, boundChuncks[0] + x, boundChuncks[1] + y, plane, 0);
					else if (plane == 0) {
						RegionBuilder.copyChunk(HouseConstants.DUNGEON[0] + (look >= 4 ? 8 : 0),
								HouseConstants.DUNGEON[1], look & 0x3, boundChuncks[0] + x, boundChuncks[1] + y, plane,
								0);
						// int regionId =
						// RegionBuilder.getRegionId(boundChuncks[0]
						// + x, boundChuncks[1] + y);

					} /*
						 * else RegionBuilder.cutChunk(boundChuncks[0] + x,
							...
							if (objects != null) {
								skip: for (WorldObject object : objects) {
									...
									if (object.getDefinitions().containsOption(4, "Build")
											|| object.getDefinitions().getName().equals("Habitat space")) {
										if (isDoor(object)) {
											int objectX = object.getChunkX() - boundChuncks[0];
											int objectY = object.getChunkY() - boundChuncks[1];
											int nextRoomX = objectX + DOOR_DIR_X[object.getRotation()];
											int nextRoomY = objectY + DOOR_DIR_Y[object.getRotation()];
											RoomReference room = getRoom(nextRoomX, nextRoomY, object.getPlane());
											if (!buildMode && object.getPlane() != 0 && room == null
													&& reference.getRoom().isShowRoof()) {
											...
											for (ObjectReference o : reference.objects) {
												...
												if (slot != -1) {
													...
													if (!buildMode && !spawnedPet) {

														if (storedPets == null) {
															storedPets = new int[5];
														}
														for (int i = 0; i < storedPets.length; i++) {
															if (storedPets[i] > 0) {
																if (getMenagerie(i) != null) {
																	if (Pets.forId(storedPets[i]) != null)
																		spawnPet(Pets.forId(storedPets[i])
																				.getBabyNpcId(), i);
																}
															}
														}
													...
													if (!buildMode
															&& HouseConstants.getMonsterId(objectR.getId()) != -1) {// 2715
														NPC npc = new NPC(HouseConstants.getMonsterId(objectR.getId()),
																new WorldTile(objectR.getX(), objectR.getY(),
																		objectR.getPlane()),
																-1, true);
														World.removeObject(objectR);
														if (World.getSpawnedObject(objectR.getX(), objectR.getY(),
														...
														if (objectR.getId() == 13356)
															objectR.setId(13361);
														else if (objectR.getId() == 13357)
															objectR.setId(13362);
														else if (objectR.getId() == 13358)
															objectR.setId(13363);
														else if (objectR.getId() == 13359)
															objectR.setId(13364);
														else if (objectR.getId() == 13360)
															objectR.setId(13365);
													}
													World.spawnObject(objectR);
													continue skip;
	...
	public void createRoom(int slot) {
		...
		if (slot >= rooms.length)
			return;
		int[] position = (int[]) owner.getTemporaryAttributtes().get("CreationRoom");
		owner.closeInterfaces();
		if (position == null)
			return;
		Room room = rooms[slot];
		if ((room == Room.DUNGEON_CORRIDOR || room == Room.DUNGEON_JUNCTION || room == Room.DUNGEON_PIT
				|| room == Room.DUNGEON_STAIRS || room == Room.TREASURE_ROOM || room == Room.OUTBLIETTE)
				&& position[2] != 0) {
			owner.getPackets().sendGameMessage("That room can only be built underground.");
			return;
		}
		if ((room == Room.GARDEN || room == Room.FORMAL_GARDEN || room == Room.MENAGERIE) && position[2] != 1) {
			owner.getPackets().sendGameMessage("That room can only be built on ground.");
			return;
		}
		...
		if (owner.getInventory().getNumberOf(995) < room.getPrice()) {
			owner.getPackets().sendGameMessage("You don't have enough coins to build this room.");
			return;
		}
		owner.getDialogueManager().startDialogue("CreateRoomD",
	...
	public void createRoom(RoomReference room) {
		if (owner.getSkills().getLevel(Skills.CONSTRUCTION) < room.room.getLevel()) {
			owner.getPackets().sendGameMessage(
					"You need a Construction level of " + room.room.getLevel() + " to build this room.");
			return;
		}
		if (owner.getInventory().getNumberOf(995) < room.room.getPrice()) {
			owner.getPackets().sendGameMessage("You don't have enough coins to build this room.");
			return;
		}
		...
		if (roomsR.add(room)) {
			if (owner.getInventory().getNumberOf(995) > room.room.getPrice())
				owner.getInventory().deleteItem(new Item(995, room.room.getPrice()));
	...
	public void destroyHouse() {
		...
		CoresManager.slowExecutor.schedule(new Runnable() {
			...
			public void run() {
				...
				// World.getRegion(RegionBuilder.getRegionId(boundChunksCopy[0],
				// boundChunksCopy[1])).getSpawnedObjects())
				// if(object != null)
	...
	public void expelGuests() {
		if (!isOwnerInside()) {
			owner.getPackets().sendGameMessage("You can only expel guests when you are in your own house.");
			return;
		}
		kickGuests();
	}
			...
			if (room.room == HouseConstants.Room.GARDEN || room.room == HouseConstants.Room.FORMAL_GARDEN) {
				for (ObjectReference o : room.objects)
					if (o.getPiece() == HouseConstants.HObject.EXIT_PORTAL)
						return new WorldTile(boundChuncks[0] * 8 + room.x * 8 + 3, boundChuncks[1] * 8 + room.y * 8 + 3,
								room.plane);
			}
		}
		// shouldnt happen
	...
	public boolean joinHouse(final Player player) {
		...
		if (!isOwner(player)) { // not owner
			if (!isOwnerInside() || !loaded || owner.blockPlayers || !owner.hasHouse) {// add
																						// the
																						// has
																						// house
																						// check
		...
		if (this.equals(null))
			return false;
		players.add(player);
		player.altarMod = owner.getHouse().litBurners;
	...
	public void leaveHouse(Player player, int type) {
		...
		if (type == LOGGED_OUT)
			player.setLocation(new WorldTile(2544, 3092, 0));// will be replaced
		else if (type == KICKED)
			player.setNextWorldTile(new WorldTile(2544, 3092, 0));// will be
																	// replaced
	...
	public void openBuildInterface(WorldObject object, final Builds build) {
		if (!buildMode) {
			owner.getDialogueManager().startDialogue("SimpleMessage", "You can only do that in building mode.");
			return;
		}
		int roomX = object.getChunkX() - boundChuncks[0];
		int roomY = object.getChunkY() - boundChuncks[1];
		...
		for (int index = 0; index < build.getPieces().length; index++) {
			...
			owner.getPackets().sendConfig(1485 + index,
					owner.getSkills().getLevel(Skills.CONSTRUCTION) >= piece.getLevel()
							&& owner.getInventory().containsItems(piece.getRequirements()) ? 1 : 0);
		}
		owner.getPackets().sendItems(-1, 0, 8, itemArray);
		owner.getPackets().sendInterSetItemsOptionsScript(interId, 11, 8, interId == 396 ? 2 : 1, 4, "Build");
		owner.getPackets().sendUnlockIComponentOptionSlots(interId, 11, 0, interId == 396 ? 7 : 3, 0);
		owner.getInterfaceManager().sendInterface(interId);
		for (int i = 0; i < (interId == 396 ? 7 : 3); i++) {
			if (i >= build.getPieces().length) {
				owner.getPackets().sendHideIComponent(interId, (interId == 394 ? 29 : 49) + i, true);
				owner.getPackets().sendIComponentText(interId, (interId == 394 ? 32 : 56) + i, "");
				owner.getPackets().sendIComponentText(interId, 14 + (5 * i), "");
				for (int i2 = 0; i2 < 4; i2++)
					owner.getPackets().sendIComponentText(interId, 15 + i2 + (5 * i), "");
			} else {
				owner.getPackets().sendIComponentText(interId, (interId == 394 ? 32 : 56) + i,
						"Lvl " + build.getPieces()[i].getLevel());
				if (build.getPieces()[i] != null)
					owner.getPackets().sendIComponentText(interId, 14 + (5 * i),
	...
	public void openRemoveBuild(WorldObject object) {
		...
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId() && getPortalCount() <= 1) {
			owner.getDialogueManager().startDialogue("SimpleMessage", "Your house must have at least one exit portal.");
			return;
		}
		int roomX = object.getChunkX() - boundChuncks[0];
		int roomY = object.getChunkY() - boundChuncks[1];
	...
	public void openRoomCreationMenu(int roomX, int roomY, int plane) {
		...
		if (room != null) {
			...
			if (roomX == 0 || roomY == 0 || roomX == 7 || roomY == 7) {
				owner.getDialogueManager().startDialogue("SimpleMessage", "You can't create a room here.");
				return;
			}
			if (plane == 2) {
			...
			for (int index = 0; index < HouseConstants.Room.values().length; index++) {
				...
				int bugged = index + 68;// these are the prices / names that are
										// bugged due to shit interface creation
				if (owner.getSkills().getLevel(Skills.CONSTRUCTION) >= refRoom.getLevel()
						&& owner.getInventory().getNumberOf(995) >= refRoom.getPrice()
						|| owner.getSkills().getLevel(Skills.CONSTRUCTION) >= refRoom.getLevel())
				...
				if (owner.getInventory().getNumberOf(995) >= 10000)
					owner.getPackets().sendIComponentText(402, 90, (owner.getInventory().getNumberOf(995) >= 10000
							? "<col=008000>10000" : "<col=cc0000>10000"));
				if (owner.getInventory().getNumberOf(995) >= 250000)
					owner.getPackets().sendIComponentText(402, 92, (owner.getInventory().getNumberOf(995) >= 250000
							? "<col=008000>250000" : "<col=cc0000>250000"));
			}
			owner.getInterfaceManager().sendInterface(402);
			owner.getTemporaryAttributtes().put("CreationRoom", new int[] { roomX, roomY, plane });
			owner.setCloseInterfacesEvent(new Runnable() {
				@Override
				public void run() {
	...
	public void previewRoom(RoomReference reference, boolean remove, boolean first) {
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				WorldObject[] objects = region.getObjects(reference.plane, (realChunkX & 0x7) * 8 + x,
						(realChunkY & 0x7) * 8 + y);
				if (objects != null) {
					for (WorldObject object : objects) {
						...
						if (defs.containsOption(4, "Build")) {
							WorldObject objectR = new WorldObject(object);
							int[] coords = DynamicRegion.translate(x, y, reference.rotation, defs.getSizeX(), defs.getSizeY(),
									object.getRotation());
							...
							if (remove) {
								...
								World.spawnObject(objectR);// spawns
																	// object,
								// doesnt spawn
								// orginal
							}
				...
				if (reference.plane == 0) {
					if (!remove) {
						if (World.getSpawnedObject(boundX + x, boundY + y, 0) != null
								&& (World.getSpawnedObject(boundX + x, boundY + y, 0).getId() == 13068
										|| World.getSpawnedObject(boundX + x, boundY + y, 0).getId() == 13026)
								|| World.getSpawnedObject(boundX + x, boundY + y, 0).getId() == 13028
								|| World.getSpawnedObject(boundX + x, boundY + y, 0).getId() == 13027)// checks
																										// if
																										// spawned
																										// object
																										// here
							World.spawnObject(new WorldObject(10, 10, 0, boundX + x, boundY + y, reference.plane), true);// spawns
						// object
						// 10 to
						// get
						// rid
						...
						if (World.getSpawnedObject(boundX + x, boundY + y, 0) != null
								&& World.getSpawnedObject(boundX + x, boundY + y, 0).getId() == 10)
							World.removeObject(World.getSpawnedObject(boundX + x, boundY + y, 0), false);
					}
				}
			}
		}
	...
	public void refreshNumberOfRooms() {
		// owner.getPackets().sendButtonConfig(944, roomsR.size());
	}
				...
				if (objects != null) {
					for (WorldObject object : objects) {
						...
						if (remove)
							...
							World.spawnObject(objectR, true);
						}
					}
				}
			}
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				if (World.getRemovedOrginalObject(0, boundX, boundY, 10) != null)
					World.spawnObject(World.getRemovedOrginalObject(0, boundX, boundY, 10), true);
			}
		}
	}
	...
	public void removeRoom() {
		...
		if (roomTo == null) {
			owner.getDialogueManager().startDialogue("SimpleMessage", "These stairs do not lead anywhere.");
			return;
		}
			...
			if (room.room == HouseConstants.Room.MENAGERIE) {
				for (ObjectReference o : room.objects)
					if (o.getPiece() == HouseConstants.HObject.OAK_PET_HOUSE
							...
							|| o.getPiece() == HouseConstants.HObject.DESECRATED_PET_HOUSE
							|| o.getPiece() == HouseConstants.HObject.NATURAL_PET_HOUSE) {
						// return new WorldTile(boundChuncks[0] * 8 + (room.x *
						// 8), boundChuncks[1] * 8 + (room.y * 8),
						// room.plane);
						switch (deviation) {
						case 0:
							return new WorldTile(boundChuncks[0] * 8 + (room.x * 8) + 2,
									boundChuncks[1] * 8 + (room.y * 8) + 4, room.plane);
						case 1:
							return new WorldTile(boundChuncks[0] * 8 + (room.x * 8) + 4,
									boundChuncks[1] * 8 + (room.y * 8) + 5, room.plane);
						case 2:
							return new WorldTile(boundChuncks[0] * 8 + (room.x * 8) + 6,
									boundChuncks[1] * 8 + (room.y * 8) + 3, room.plane);
						case 3:
							return new WorldTile(boundChuncks[0] * 8 + (room.x * 8) + 6,
									boundChuncks[1] * 8 + (room.y * 8) + 1, room.plane);
						case 4:
						default:
							return new WorldTile(boundChuncks[0] * 8 + (room.x * 8) + 8,
									boundChuncks[1] * 8 + (room.y * 8) + 3, room.plane);
						}
					}
			}
