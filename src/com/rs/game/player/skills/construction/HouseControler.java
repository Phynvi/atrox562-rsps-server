...
import com.rs.game.player.actions.construction.SitBench;
import com.rs.game.tasks.WorldTask;
import com.rs.game.tasks.WorldTasksManager;
import com.rs.game.WorldTile;
import com.rs.game.player.actions.construction.SitChair;
import com.rs.game.player.actions.construction.SitThrone;
import com.rs.game.player.dialogues.StorePet;
...
public class HouseControler extends Controler {
	...
	public boolean canDropItem(Item item) {
		//if (house.isBuildMode()) {
			player.getPackets().sendGameMessage("You cannot drop items here.");
			return false;
		//}
		//return true;
	}
			...
			if (ring == HouseConstants.HObject.BOXING_RING) {
				if (slotId == Equipment.SLOT_WEAPON) {
					if (itemId != 7671 && itemId != 7673) {
						player.sendMessage("You can't use this weapon in here.");
						return false;
					}
				} else {
					player.sendMessage("You can't this in here.");
					return false;
				}
			} else if (ring == HouseConstants.HObject.FENCING_RING) {
				if (slotId != Equipment.SLOT_WEAPON) {
					player.sendMessage("You can't equip this in here.");
					return false;
				}
			}
		}
	...
	@Override
	public boolean checkWalkStep(final int lastX, final int lastY, int nextX, int nextY) {
		WorldObject tile = World.getObject(new WorldTile(nextX, nextY, player.getPlane()), 22);
		// WorldObject lastTile = World.getObject(new WorldTile(lastX, lastY,
		// player.getPlane()), 22);
		if (tile != null) {
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		if (object.getId() >= HouseConstants.HObject.OAK_PET_HOUSE.getId()
				&& object.getId() <= HouseConstants.HObject.NATURAL_PET_HOUSE.getId()) {
			if (Pets.forId(item.getId()) != null) {
				player.getDialogueManager().startDialogue("StorePet", house, StorePet.ITEM, item.getId());
			} else
				player.sendMessage("That is not a pet...");
			return false;
		}
		if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId()
				|| object.getId() == HouseConstants.HObject.STONE_FIREPLACE.getId()
				|| object.getId() == HouseConstants.HObject.MARBLE_FIREPLACE.getId()) {
			if (item.getId() != 1511) {
			 ...
			 * {TODO Cookables cook = Cooking.isCookingSkill(item); if (cook !=
			 * null) { player.getDialogueManager().startDialogue("CookingD",
			 * cook, object); return false; } player.getDialogueManager()
			 * .startDialogue( "SimpleMessage", "You can't cook that on a " +
			 * (object.getDefinitions().name .equals("Fire") ? "fire" : "range")
			 * + "."); return false; }
			 */
		return true;
	}
	...
	public boolean login() {
		player.setNextWorldTile(new WorldTile(2543, 3096, 0));
	...
	public void process() {
		...
		final WorldObject trap = World.getObject(new WorldTile(player.getX(), player.getY(), player.getPlane()), 22);
		if (trap != null) { // 75
			...
			if (trap.getId() == 13361) {
				...
				player.applyHit(new Hit(player, Utils.random(100), HitLook.MELEE_DAMAGE));
				player.resetWalkSteps();
				World.removeObject(trap, false);
				CoresManager.slowExecutor.schedule(new Runnable() {
					@Override
					public void run() {
						try {
							World.spawnObject(trap, true);
						} catch (Throwable e) {
							Logger.handle(e);
						}
					}
				...
				player.applyHit(new Hit(player, Utils.random(200), HitLook.MELEE_DAMAGE));
				player.resetWalkSteps();
				World.removeObject(trap, false);
				CoresManager.slowExecutor.schedule(new Runnable() {
					@Override
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
		...
		else if (object.getId() >= HouseConstants.HObject.OAK_LECTURN.getId()
				&& object.getId() <= HouseConstants.HObject.MAHOGANY_DEMON_LECTURN.getId()) {
			player.getInterfaceManager().sendInterface(400);
		} else if (object.getId() >= HouseConstants.HObject.OAK_PET_HOUSE.getId()
				&& object.getId() <= HouseConstants.HObject.NATURAL_PET_HOUSE.getId()) {
			...
			player.getDialogueManager().startDialogue("HouseShelves");
		} else if (object.getId() >= HouseConstants.HObject.INSCENCE_BURNER.getId()
				&& object.getId() <= HouseConstants.HObject.MARBLE_BURNER.getId()) {
			if (player.getInventory().containsItem(590, 1) && player.getInventory().containsItem(251, 1)) {
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (loop == 0) {
							player.lock(1);
							player.sendMessage(
									"You attempt to light the " + object.getDefinitions().getName().toLowerCase() + "...");
							...
							for (Player player : house.getPlayers()) {
								if (player == null || player.hasFinished())
									continue;
								player.altarMod = house.litBurners;
							}
							World.removeObject(new WorldObject(object.getId(), object.getType(), object.getRotation(),
									object.getX(), object.getY(), object.getPlane()), true);
							World.spawnObject(new WorldObject(object.getId() + 1, object.getType(), object.getRotation(),
									object.getX(), object.getY(), object.getPlane()), true);
							player.sendMessage("It catches alight.");
						} else if (loop == 130) {
							house.litBurners--;
							...
							World.removeObject(new WorldObject(object.getId() + 1, object.getType(),
									object.getRotation(), object.getX(), object.getY(), object.getPlane()), true);
							World.spawnObject(new WorldObject(object.getId(), object.getType(), object.getRotation(),
									object.getX(), object.getY(), object.getPlane()), true);
							// stop();
						}
						loop++;
				...
				player.sendMessage("You need a tinderbox and a clean marrentill to light this.");
			}
		} else if (object.getId() >= HouseConstants.HObject.WOODEN_TORCHES.getId()
				&& object.getId() <= HouseConstants.HObject.GOLD_CANDLESTICK.getId()) {
			if (player.getInventory().containsItem(590, 1)) {
				WorldTasksManager.schedule(new WorldTask() {
					int loop;
		...
		} else if (object.getId() >= HouseConstants.HObject.CLAY_FIREPLACE.getId()
				&& object.getId() <= HouseConstants.HObject.MARBLE_FIREPLACE.getId()) {
			if (player.getInventory().containsItem(590, 1) && player.getInventory().containsItem(1511, 1)) {
				...
				player.sendMessage("You need a tinderbox and some logs to light this.");
			}
		} else if (object.getId() >= HouseConstants.HObject.OAK_ALTAR.getId()
				&& object.getId() <= HouseConstants.HObject.GILDED_ALTAR.getId()) {
			if (object.getDefinitions().containsOption(0, "Pray")
					|| object.getDefinitions().containsOption(0, "Pray-at")) {
			...
			player.getDialogueManager().startDialogue("Transportation", "Edgeville", new WorldTile(3087, 3496, 0),
					"Karamja", new WorldTile(2918, 3176, 0), "Draynor Village", new WorldTile(3105, 3251, 0), "Al Kharid",
					new WorldTile(3293, 3163, 0), null);
		} else if (object.getId() >= HouseConstants.HObject.CRUDE_WOODEN_CHAIR.getId()
				&& object.getId() <= HouseConstants.HObject.MAHOGANY_ARMCHAIR.getId()) {
			...
			player.getActionManager().setAction(new SitChair(player, chair, object));
				...
				&& object.getId() <= HouseConstants.HObject.DEMONIC_THRONE.getId()) {
			int throne = object.getId() - HouseConstants.HObject.OAK_THRONE.getId();
			player.getActionManager().setAction(new SitThrone(player, throne, object));
			...
			player.getActionManager().setAction(new SitBench(player, bench, object));
		} else if (object.getId() == HouseConstants.HObject.SCRYING_POOL.getId()) {// as
																					// a
																					// test
			...
			if (object.getDefinitions().getOption(1).equals("Climb"))
				...
				house.climbStaircase(object, object.getDefinitions().getOption(1).equals("Climb-up"), player);
		} else if (object.getId() == HouseConstants.HObject.DUNGEON_ENTRACE.getId()) {
			house.climbStaircase(object, object.getDefinitions().getOption(1).equals("Climb-down"), player);
		} else if (object.getId() == HouseConstants.HObject.OAK_DOOR.getIds()[0]
				|| object.getId() == HouseConstants.HObject.OAK_DOOR.getIds()[1]) {
			player.sendMessage("You attempt to open the door...");
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					this.stop();
					if (Utils.random(5) == 1) {
						player.sendMessage("You manage to pry the door open for a moment. Quick! Get through!");
						handleDoor(player, object, 3000);
					} else
						player.sendMessage("The door is locked, you couldnt get it open.");
				}
			}, 0, 2);
		...
		} else if (object.getId() == HouseConstants.HObject.MARBLE_DOOR.getIds()[0]
				|| object.getId() == HouseConstants.HObject.MARBLE_DOOR.getIds()[1]) {
			player.sendMessage("You attempt to open the door...");
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
			...
			if (checkRing(object.getId()) == HouseConstants.HObject.BOXING_RING) {
				if (player.getEquipment().wearingArmourNoWeapon()) {
					player.sendMessage("Please take all your armour off before entering this ring.");
					return false;
				} else if (player.getEquipment().hasWeapon()
						&& !(player.getEquipment().getItem(Equipment.SLOT_WEAPON).getId() == 7671
								|| player.getEquipment().getItem(Equipment.SLOT_WEAPON).getId() == 7673)) {
					player.sendMessage("Please remove any weapons, boxing gloves are allowed.");
					return false;
				}
			...
			final WorldTile[] tile = new WorldTile[] {
					insideCombatRing ? new WorldTile(player.getX() + 1, player.getY(), player.getPlane())
							: new WorldTile(player.getX() - 1, player.getY(), player.getPlane()),
					insideCombatRing ? new WorldTile(player.getX(), player.getY() - 1, player.getPlane())
							: new WorldTile(player.getX(), player.getY() + 1, player.getPlane()),
					insideCombatRing ? new WorldTile(player.getX() - 1, player.getY(), player.getPlane())
							: new WorldTile(player.getX() + 1, player.getY(), player.getPlane()),
					insideCombatRing ? new WorldTile(player.getX(), player.getY() + 1, player.getPlane())
							: new WorldTile(player.getX(), player.getY() - 1, player.getPlane()) };

			if ((World.getObject(tile[object.getRotation()], 22) == null) && !insideCombatRing) {
				player.sendMessage("You can't get in from here.");
				return false;
			}
			player.lock();
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (stage == 0) {
						...
						player.setNextForceMovement(new ForceMovement(player, 1, tile[object.getRotation()], 2,
	...
	public boolean processObjectClick2(final WorldObject object) {
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
			...
			house.climbStaircase(object, true, player);
				...
				|| object.getId() == HouseConstants.HObject.OAK_DOOR.getIds()[1]) {
			player.sendMessage("You pick lock the door");
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(17) >= 55 && Utils.random(1) == 0) {
						player.sendMessage(
								"The locks seem to slip, and the door opens for a moment. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
						player.sendMessage("The locks built into the door are too strong. You failed opening them.");
				}
			}, 0, 2);
		} else if (object.getId() == HouseConstants.HObject.STEEL_DOOR.getIds()[0]
				|| object.getId() == HouseConstants.HObject.STEEL_DOOR.getIds()[1]) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(17) >= 65 && Utils.random(1) == 0) {
						player.sendMessage(
								"The locks seem to slip, and the door opens for a moment. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(17) >= 75 && Utils.random(1) == 0) {
						player.sendMessage(
								"The locks seem to slip, and the door opens for a moment. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
	...
	public boolean processObjectClick3(final WorldObject object) {
		if (HouseConstants.Builds.STAIRCASE.containsObject(object)
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(2) >= 55 && Utils.random(1) == 0) {
						player.sendMessage("You use your strength to smash through the locks. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
						player.sendMessage(
								"The locks built into the door are too strong for you to break through. You failed opening them.");
				}
			}, 0, 2);
		} else if (object.getId() == HouseConstants.HObject.STEEL_DOOR.getIds()[0]
				|| object.getId() == HouseConstants.HObject.STEEL_DOOR.getIds()[1]) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(2) >= 65 && Utils.random(1) == 0) {
						player.sendMessage("You use your strength to smash through the locks. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
						player.sendMessage(
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					this.stop();
					if (player.getSkills().getLevel(2) >= 75 && Utils.random(1) == 0) {
						player.sendMessage("You use your strength to smash through the locks. Quick! Get through!");
						handleDoor(player, object, 5000);
					} else
						player.sendMessage(
	...
	public boolean processObjectClick5(WorldObject object) {
		if (HouseConstants.Builds.STAIRCASE.containsObject(object)
				|| HouseConstants.Builds.STAIRCASE_DOWN.containsObject(object)
				|| object.getId() == HouseConstants.HObject.DUNGEON_ENTRACE.getId())
			house.removeRoom();
		return false;
	}
	...
	public boolean processObjectClick4(WorldObject object) {
		if (object.getDefinitions().containsOption(4, "Build")) {
			...
			if (!house.isOwner(player)) {
				...
				player.sendMessage("You can only do this outside of a combat ring.");
				return false;
			}
			player.faceObject(object);
	...
	public void start() {
		...
		getArguments()[0] = null;
		// its was gonna be saved unless somehow in a
