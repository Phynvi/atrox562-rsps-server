package com.rs.game.player.skills.farming;
...
import com.rs.game.player.skills.woodcutting.Woodcutting.HatchetDefinitions;
...
public class FarmingManager implements Serializable {
	...
	private static final int[][] HARVEST_AMOUNTS = { { 3, 53 }, { 1, 1 }, { 3, 41 }, { 1, 3 }, { 3, 5 }, { 3, 5 },
			{ 3, 18 } };
	private static final int[] COMPOST_ORGANIC = { 6055, 1942, 1957, 1965, 5986, 5504, 5982, 249, 251, 253, 255, 257,
			2998, 259, 261, 263, 3000, 265, 2481, 267, 269, 1951, 753, 2126, 247, 239, 6018 };
	private static final int[] SUPER_COMPOST_ORGANIC = { 2114, 5978, 5980, 5982, 6004, 247, 6469 };
	...
	public static enum SpotInfo {
		...
		Talvery_Tree(8388, TREES), Falador_Garden_Tree(8389, TREES), Varrock_Tree(8390, TREES), Lumbridge_Tree(8391,
				TREES), Gnome_Tree(19147, TREES),

		Gnome_Strong_Fruit_Tree(8395, FRUIT_TREES), Gnome_Fruit_Tree(7963, FRUIT_TREES), Brimhaven_Fruit_Tree(7964,
				FRUIT_TREES), Catherby_Fruit_Tree(7965, FRUIT_TREES), Lletya_Fruit_Tree(28919, FRUIT_TREES),

		Falador_Allotment_North(8550, ALLOTMENT), Falador_Allotment_South(8551, ALLOTMENT), Catherby_Allotment_North(
				8552, ALLOTMENT), Catherby_Allotment_South(8553, ALLOTMENT), Ardougne_Allotment_North(8554,
						ALLOTMENT), Ardougne_Allotment_South(8555, ALLOTMENT), Canfis_Allotment_North(8556,
								ALLOTMENT), Canfis_Allotment_South(8557, ALLOTMENT),
		...
		Falador_Flower(7847, FLOWERS), Catherby_Flower(7848, FLOWERS), Ardougne_Flower(7849,
				FLOWERS), Canfis_Flower(7850, FLOWERS),
		...
		Falador_Herb_Patch(8150, HERBS), Catherby_Herb_Patch(8151, HERBS), Ardougne_Herb_Patch(8152,
				HERBS), Canfis_Herb_Patch(8153, HERBS), Trollheim_Herb_Patch(18816, HERBS),
		...
		private SpotInfo(int objectId, int type) {
			...
			this.configFileId = ObjectDefinitions.getObjectDefinitions(objectId).varbit;
			this.type = type;
		}
	}
	...
	private void handleFarming(SpotInfo info, Item item, int optionId) {
		...
		if (!spot.isCleared()) {
			...
			if (item != null) {
				...
				else if (!player.getInventory().containsItem(5343, 1)) {
					player.sendMessage(
							"You need a seed dibber in order to plant " + item.getName().toLowerCase() + "s.");
					return;
				}
				...
				case 1:
					if (info.type == TREES) {
						...
						if (spot.reachedMaxStage() && !spot.hasChecked())
							...
							collectTreeProducts(spot, TreeDefinitions.FRUIT_TREE);
						...
						if (spot.reachedMaxStage() && !spot.hasChecked()) {
							...
							player.getDialogueManager().startDialogue("SimpleMessage",
									"The weeds haven't finished decomposing yet.");
						else
	...
	private void startHarvestingAction(final FarmingSpot spot) {
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean process(Player player) {
				if (spot.harvestAmount > 0)
					...
					player.getPackets().sendGameMessage(
							"You harvest the produce in the " + patchName + " patch and get it ready for new crops.");
					player.setNextAnimation(new Animation(-1));
					spot.setIdle();
					return false;
				}
			...
			public int processWithDelay(Player player) {
				...
				if (!player.getInventory().addItem(spot.productInfo.productId, 1)) {
					World.addGroundItem(new Item(spot.productInfo.productId, 1), new WorldTile(player), player, true,
							180, true);
				}
				return 2;
			}
	...
	private void clearCompostAction(final FarmingSpot spot) {
		player.getActionManager().setAction(new Action() {
			...
			public int processWithDelay(Player player) {
				...
				if (!player.getInventory().addItem(spot.getCompost() ? 6032 : 6034, 1)) {
					World.addGroundItem(new Item(spot.getCompost() ? 6032 : 6034, 1), new WorldTile(player), player,
							true, 180, true);
				}
				spot.harvestAmount--;
				spot.refresh();
	...
	private void clearFarmingPatch(final FarmingSpot spot) {
		final String patchName = getPatchName(spot.productInfo.type);
		player.getActionManager().setAction(new Action() {
			...
			public boolean process(Player player) {
				if (stage != 2)
					...
					player.getPackets()
							.sendGameMessage("You clear the " + patchName + " and get it ready for new crops.");
					player.setNextAnimation(new Animation(-1));
					spot.setIdle();
					return false;
				}
	...
	private boolean startFarmingCycle(FarmingSpot spot, Item item) {
		...
		if (!player.getInventory().containsItem((isTree ? 952 : 5343), 1)) {
			player.getPackets()
					.sendGameMessage(isTree ? "You need a spade to plant the sappling into the " + patchName + " patch."
							: "You need a seed dibber to plant the seed in the " + patchName + " patch.");
			return true;
	...
	private boolean startWateringAction(final FarmingSpot spot, Item item) {
	...
	private boolean startCureAction(final FarmingSpot spot, final Item item) {
		...
		if (!spot.isDiseased()) {
			player.getPackets()
					.sendGameMessage("The produce in this patch isn't diseased and it doesn't need to be cured.");
			return true;
		} else if (isTree || isBush) {
			if (!(player.getInventory().containsItem(5329, 1) || !player.getInventory().containsItem(7409, 1))) {
				player.getPackets().sendGameMessage("You need some secateurs to prune this "
						+ spot.productInfo.name().toLowerCase() + " " + (isBush ? "bush" : "tree") + ".");
			}
		}
		player.getPackets()
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (!isTree && !isBush) {
					...
					player.setNextAnimation(new Animation(-1));
				player.getPackets()
						.sendGameMessage("The produce in this patch has been restored to its natural health.");
				spot.refresh();
			}
		}, 2);
		return true;
	...
	private boolean startCompostAction(final FarmingSpot spot, final Item item, boolean superCompost) {
		if (spot == null || spot.spotInfo.type == COMPOST)
			return false;
		if (spot.hasCompost()) {
			player.getPackets()
					.sendGameMessage("This patch is saturated by " + (superCompost ? "supercompost" : "compost") + ".");
			return true;
		...
		player.getPackets()
				.sendGameMessage("You saturate the patch with " + (superCompost ? "supercompost" : "compost") + ".");
		...
		player.getActionManager().setAction(new Action() {
			...
			private boolean checkAll(Player player) {
				for (HatchetDefinitions def : HatchetDefinitions.values()) {
					if (player.getInventory().containsItem(def.getItemId(), 1)
			...
			public int processWithDelay(Player player) {
				...
				player.sendMessage("You get some " + logName + ".");
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (item == null || !player.getInventory().containsItem(item.getId(), 1) || spot.isCleared())
					...
					player.getPackets().sendGameMessage("You cannot add " + item.getId() + " "
							+ (item.getAmount() > 1 ? "s" : "") + " to the compost bin.");
					return false;
				}
				return true;
			}
	...
	private void sendNeedsWeeding(boolean cleared) {
		player.getPackets().sendGameMessage(cleared ? "The patch is ready for planting." : "The patch needs raking.");
	}
	...
	public class FarmingSpot implements Serializable {
		...
		private FarmingSpot(SpotInfo spotInfo) {
		...
		private void setCycleTime(boolean reset, long cycleTime) {
			if (reset)
				this.cycleTime = 0;
			if (this.cycleTime == 0)
		...
		private boolean hasGivenAmount() {
			return attributes[9];
		}
		...
		private boolean hasEmptyHarvestAmount() {
