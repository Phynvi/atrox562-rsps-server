...
public class Smithing extends Action {
	...
	private Smithing(int ticks, int index) {
		this.index = index;
		this.ticks = ticks;
	}
	...
	public boolean process(Player player) {
		...
		if (!player.getInventory().containsItem(HAMMER, 1)) {
			player.sendMessage("You need a hammer in order to work with a bar of "
		...
		if (player.getSkills().getLevel(Skills.SMITHING) < ForgingInterface.getLevels(bar, index, player)) {
			player.sendMessage("You need a Smithing level of " + ForgingInterface.getLevels(bar, index, player)
					+ " to create this.");
			return false;
		}
	...
	public int processWithDelay(Player player) {
		...
		player.getInventory().deleteItem(bar.getBarId(),
				ForgingInterface.getActualAmount(
						bar.getLevel() + ForgingInterface.getFixedAmount(bar, bar.getItems()[index]), bar,
						bar.getItems()[index].getId()));
	...
	public static class ForgingInterface {
		...
		private static final int componentChilds[] = new int[30];
		private static final int CLICKED_CHILDS[] = { 28, -1, 5, 1 };
		...
		private static int getForgedAmount(int id) {
		...
		private static String[] getStrings(Player player, ForgingBar bar, int index, int itemId) {
		...
		private static int getLevels(ForgingBar bar, int slot, Player player) {
		...
		private static int getActualAmount(int levelRequired, ForgingBar bar, int id) {
			if (levelRequired >= 99) {
				levelRequired = 99;
			}
			int level = levelRequired - bar.getLevel();
			String name = ItemDefinitions.getItemDefinitions(id).getName().toLowerCase();
			if (name.contains("longsword") || name.contains("sq shield"))
				return 2;
			if (name.contains("2h"))
				return 3;
			if (level >= 0 && level <= 4) {
		...
		public static void sendSmithingInterface(Player player, ForgingBar bar) {
			...
			for (int i = 0; i < bar.getItems().length; i++) {
				int amount = 1;
				if (bar.getItems()[i].getDefinitions() != null) {
					if (bar.getItems()[i].getDefinitions().getName().contains("arrowtip")
							|| bar.getItems()[i].getDefinitions().getName().contains("nail"))
						amount = 15;
					else if (bar.getItems()[i].getDefinitions().getName().contains("bolt")
							|| bar.getItems()[i].getDefinitions().getName().contains("dart"))
						amount = 10;
					else if (bar.getItems()[i].getDefinitions().getName().contains("knife"))
						amount = 5;
				}
				player.getPackets().sendItemOnIComponent(SMITHING_INTERFACE, componentChilds[i],
						bar.getItems()[i].getId(), amount);
