...
public class GemCutting extends Action {
	...
	public static void cut(Player player, Gem gem) {
		...
		if (amount <= 1)
			...
			if (new GemCutting(gem, amount).checkAll(player))
				player.getActionManager().setAction(new GemCutting(gem, 28));
		}	
	}
	...
	private boolean isSuccesfull(Player player) {
		...
		if (gem != Gem.ONYX && (Math.round(ratio * craftingLevel) < gem.getLevelRequired()))
			return false;
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		player.getSkills().addXp(Skills.CRAFTING, gem.getExperience() / 10);
		player.getPackets().sendGameMessage(
				"You cut the "
		...
		if (quantity <= 0)
			return -1;
		player.setNextAnimationNoPriority(new Animation(gem.getEmote()), player); // start the
		return 1;
	}
