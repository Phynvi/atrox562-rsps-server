...
public final class EmotesManager implements Serializable {
	...
	public EmotesManager() {
		...
		for (int emoteId = 2; emoteId < 24; emoteId++) //24
		...
		if (!unlockedEmotes.contains(id) /*&& player.getRights() < 2*/) {
			...
			if (id == 2) {
				...
				case 10647: // Hitpoints cape
					player.setNextAnimation(new Animation(4971));
					player.setNextGraphics(new Graphics(833));
					break;
				case 9756:
				case 9757:
				...
				player.getPackets().sendMusic(302);
			} else if (id == 42) {
			...
			else if (ClueScrolls.completedRiddle(player, id)) {
				return;
			}
			setNextEmoteEnd();
		}
	...
	public void refreshListConfigs() {
		if (unlockedEmotes.contains(24) && unlockedEmotes.contains(25))
			player.getPackets().sendConfig(465, 7); // goblin quest emotes unlocked = 7
		int value1 = 0;
