...
public enum Pets {
	 ...
	 * Boss pets.
	 */
	 DAGANNOTH_SUPREME_JR(15297, -1, -1, 9343, -1, -1, 0.0, 99),
	 DAGANNOTH_PRIME_JR(15298, -1, -1, 9344, -1, -1, 0.0, 99),
	 DAGANNOTH_REX_JR(15299, -1, -1, 9326, -1, -1, 0.0, 99),
	 SECOND_KALPHITE(15300, -1, -1, 9334, -1, -1, 0.0, 99),
	 PRINCE_BLACK_DRAGON(15301, -1, -1, 9333, -1, -1, 0.0, 99),
	 PET_KRAKEN(15302, -1, -1, 9308, -1, -1, 0.0, 99),
	 PET_PENANCE(15303, -1, -1, 9327, -1, -1, 0.0, 99),
	 PET_CHAOS_ELE(15304, -1, -1, 9345, -1, -1, 0.0, 99);
	...
	public static boolean talkTo(Player player, NPC npc) {
		if (npc instanceof Pet) {
			...
			if (npc.getId() == /*9325*/player.getPet().getId()) {
					player.getDialogueManager().startDialogue("KrakenPet", npc.getId());
                    return true;
				} else if (npc.getId() == /*9329*/player.getPet().getId()) {
					player.getDialogueManager().startDialogue("ChaosElePet", npc.getId());
                    return true;
				} else if (npc.getId() == /*9326*/player.getPet().getId()) {
					player.getDialogueManager().startDialogue("PenancePet", npc.getId());
                    return true;
				} else if (npc.getId() == /*9323*/player.getPet().getId()) {
					player.getDialogueManager().startDialogue("PrinceBlackDragon", npc.getId());
                    return true;
				} else if (npc.getId() == /*9317*/player.getPet().getId()) {
					player.getDialogueManager().startDialogue("KalphitePrincess", npc.getId());
                    return true;
				}
			player.getDialogueManager().startDialogue("SimpleMessage", "This pet does'nt have a dialogue yet, report to admin.");
		}
		return false;
	}
		...
		if (npc instanceof Pet) {
			...
			if (npc.getId() < 15980) {
				if (!player.getInventory().hasFreeSlots()) {
					player.sendMessage("You do not have enough inventory space to pickup your pet.");
					return true;
				}
			...
			player.getDialogueManager().startDialogue("SimpleMessage", "I better not touch it..");
		}
		return false;
	}
