...
public class Sawmill {
	...
	public enum Plank {
		WOOD(ItemIdentifiers.WOODEN_PLANK, ItemIdentifiers.LOGS, 100),
		OAK(ItemIdentifiers.OAK_PLANK, ItemIdentifiers.OAK_LOGS, 250),
		TEAK(ItemIdentifiers.TEAK_PLANK, ItemIdentifiers.TEAK_LOGS, 500),
		MAHOGANY(ItemIdentifiers.MAHOGANY_PLANK, ItemIdentifiers.MAHOGANY_LOGS, 1500);
	...
	private static Plank getPlank(int id) {
		for (Plank plank : Plank.values())
			if (plank.id == id)
				return plank;
		return null;
