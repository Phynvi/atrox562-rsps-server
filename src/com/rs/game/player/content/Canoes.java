...
public class Canoes {
    ...
    public static boolean chopCanoeTree(Player player, final int configIndex) {
        player.getActionManager().setAction(new Action() {
            ...
            private boolean checkAll(Player player) {
                for (Woodcutting.HatchetDefinitions def : Woodcutting.HatchetDefinitions.values()) {
                    if (player.getInventory().containsItem(def.getItemId(), 1) || player.getEquipment().getWeaponId() == def.getItemId()) {
                        hatchet = def;
                ...
                if (hatchet == null) {
                    hatchet = Woodcutting.HatchetDefinitions.BRONZE;
                    player.getPackets()
                            .sendGameMessage(
            ...
            public int processWithDelay(Player player) {
                ...
                player.getVarBitManager().sendVarbit(1839 + configIndex, 10);
                isComplete = true;
                stop(player);
                return 4;
            }
    ...
    public static void createShapedCanoe(final Player player) {
        ...
        Woodcutting.HatchetDefinitions hatchet = null;
        for (Woodcutting.HatchetDefinitions def : Woodcutting.HatchetDefinitions.values()) {
    ...
    public static void deportCanoeStation(final Player player, int selectedArea) {
        ...
        int canoeArea = (int) player.getTemporaryAttributtes().get(
                "selected_area");
        System.out.println("Canoe Area = " + canoeArea);
        if (selectedArea != canoeArea) {
            if (selectedArea > (canoeArea + selectedCanoe + 1)
                    || selectedArea < (canoeArea - selectedCanoe - 1)) {
                player.getPackets().sendGameMessage("This is too far to reach, please pick another plot point or make a better canoe.");
                ...
                player.closeInterfaces();
                player.getVarBitManager().sendVarbit((int) player.getTemporaryAttributtes().get("canoe_config"), 0);
