...
import com.rs.game.player.skills.smithing.Smithing;
...
public class GodswordCreating {
	...
	public static void joinPieces(Player player, boolean anvil) {
		if (!anvil) {
			...
			player.getSkills().addXp(Skills.SMITHING, 20);
	...
	public static void attachHilt(Player player, int gs) {
		...
		player.getPackets().sendGameMessage("You attach the hilt to the blade and make a " + ItemDefinitions.getItemDefinitions(gsId).getName() + ".");
	}
