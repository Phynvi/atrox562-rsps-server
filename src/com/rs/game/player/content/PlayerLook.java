...
public final class PlayerLook {
	...
	public static void openShoeCustumization(final Player player) {
		if(player.getEquipment().getBootsId() != -1) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 1301, "you'll have to take what your wearing on your feet off first.");
			return;
		}
		...
		player.setCloseInterfacesEvent(new Runnable() {
			...
			@Override
			public void run() {
				player.getDialogueManager().startDialogue("SimpleNPCMessage", 1301, "Nice choice!");
	...
	public static void openMageMakeOver(Player player) {
		...
		player.getPackets().sendConfig(6098,
				player.getAppearence().isMale() ? 0 : 1);
		player.getPackets().sendConfig(6099,
	...
	public static void handleThessaliasMakeOverButtons(Player player, int buttonId, int slotId) {
		if (buttonId == 6)
			...
			if (stage == null || stage == 0 || stage == 1)
				player.getAppearence().setTopColor(ClientScriptMap.getMap(2348).getIntValue(slotId - 29));
			else if (stage == 3)
				player.getAppearence().setLegsColor(ClientScriptMap.getMap(2348).getIntValue(slotId - 29));
		}
	}
	...
	public static void openThessaliasMakeOver(final Player player) {
		if(player.getEquipment().wearingArmour()) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 548, "You're not able to try on my clothes with all that armour.<br>Take it off and then speak to me again.");
			return;
		}
		showBooth(player);
		player.setNextAnimation(new Animation(11623));
