...
public class Banker extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if(stage == -1 && !LendingManager.hasLendedOut(player)) {
			stage = 0;
			sendDialogue(SEND_4_OPTIONS,
					...
					,"What is this place?");
		} else if(stage == -1 && LendingManager.hasLendedOut(player)) {
			stage = 6;
			sendDialogue(SEND_4_OPTIONS,
					"What would you like to say?",
					"I'd like to access my bank account, please."
		...
		//}else
			...
			if(componentId == 1) {
				...
				LendingManager.ReturnItem(player);
				//LendingManager.ReturnedItems(player);
			}else
				end();
	}
}
