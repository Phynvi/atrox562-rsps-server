...
public class RangingMaster extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if(stage == -1) {
			...
			sendEntityDialogue(SEND_1_TEXT_CHAT,
					new String[] { player.getDisplayName(), "I would like to buy a ranging skillcape." },
                ...
                if (player.getSkills().getLevel(Skills.RANGE) == 99) {
                	...
                	sendEntityDialogue(SEND_3_TEXT_CHAT, new String[] {
							NPCDefinitions.getNPCDefinitions(npcId).name,
							"Anybody who has spent as much time ranging as", 
                	...
                	sendEntityDialogue(SEND_2_TEXT_CHAT, new String[] {
							NPCDefinitions.getNPCDefinitions(npcId).name,
							"Looks like you don't have a level of 99 ranged,",
							"you need a level of 99 ranged to buy one."
							}, IS_NPC, npcId, 9827);;
                }
		}else if(stage == 100) {
end();
			...
			if(componentId == 1) {
				...
				stage = 100;
					sendEntityDialogue(SEND_1_TEXT_CHAT, new String[] {
							NPCDefinitions.getNPCDefinitions(npcId).name,
							"Alright come back when you can afford one."
							}, IS_NPC, npcId, 9827);
			}else if (stage == 4) {
				stage = 100;
				...
				player.getInventory().addItem(9756,1);
				player.getInventory().addItem(9758,1);
				}
				}else
				end();
