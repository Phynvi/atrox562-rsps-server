...
public class CreateRoomStairsD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			Room r = room.getPlane() == 1 && !up && componentId == OPTION_3 ? Room.DUNGEON_STAIRS
					: componentId == OPTION_2 ? up ? Room.HALL_QUEST_DOWN : Room.HALL_QUEST
							: up ? Room.HALL_SKILL_DOWN : Room.HALL_SKILL;

			Builds stair = room.getPlane() == 1 && !up && componentId == OPTION_3 ? Builds.STAIRCASE_2
					: componentId == OPTION_2 ? up ? Builds.STAIRCASE_DOWN_1 : Builds.STAIRCASE_1
							: up ? Builds.STAIRCASE_DOWN : Builds.STAIRCASE;

			RoomReference newRoom = new RoomReference(r, room.getX(), room.getY(), room.getPlane() + (up ? 1 : -1),
					room.getRotation());
	...
	public void start() {
		...
		sendOptionsDialogue("These stairs do not lead anywhere. Do you want to build a room at the "
