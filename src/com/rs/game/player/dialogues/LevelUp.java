...
public final class LevelUp extends Dialogue {
	...
	public static final int[] SKILL_ICON = { 100000000, 400000000, 200000000, 450000000, 250000000, 500000000,
			300000000, 1100000000, 1250000000, 1300000000, 1050000000, 1200000000, 800000000, 1000000000, 900000000,
			650000000, 600000000, 700000000, 1400000000, 1450000000, 850000000, 1500000000, 1600000000, 1650000000, 0 };
	...
	public void start() {
		...
		player.getPackets().sendIComponentText(740, 1,
				"You have now reached level " + player.getSkills().getLevelForXp(skill) + ".");
		player.getPackets().sendGameMessage("You've just advanced a" + (name.startsWith("A") ? "n" : "") + " " + name
				+ " level! You have reached level " + player.getSkills().getLevelForXp(skill) + ".");
	...
	public static void send104m(Player player, int skill) {
		World.sendWorldMessage("<img=9> <col=FFFFFF><shad=00FFFF>[News] " + player.getDisplayName() + " has just reached 104M experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	...
	public static void sendNews(Player player, int skill, int level) {
		...
		if (reachedAll) {
			player.getPackets().sendGameMessage("Contratulations you have reached the maximum possible total level.");
		}
		if (!reachedAll) {
			World.sendWorldMessage("<img=4><col=e112a4>News: " + player.getDisplayName() + " has achieved " + level + " " + Skills.SKILL_NAME[skill] + ".", false);
			player.getPackets().sendGameMessage("You have achieved highest possible level in " + Skills.SKILL_NAME[skill] + ".");
		} else if (reachedAll) {
			World.sendWorldMessage("<img=4><col=e112a4>News: " + player.getDisplayName() + " has achieved " + level + " "+ Skills.SKILL_NAME[skill] + ".", false);
			player.getPackets().sendGameMessage("You have achieved highest possible level in "+ Skills.SKILL_NAME[skill] + ".");
			World.sendWorldMessage("<img=4><col=ff0000>News: " + player.getDisplayName() + " has just achieved at least level 99 in all skills!", false);
			player.getPackets().sendGameMessage("You have achieved highest possible level in all skills.");
		}
	}
