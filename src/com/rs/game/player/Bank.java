...
public class Bank implements Serializable {
	...
	public void checkPinInput(boolean isConfirmation, int componentId) {
		...
		if (stage++ == 4) {
			...
			if (actualPin == null || requestedPin != actualPin) { // either
				...
				player.getPackets().sendGameMessage("The PIN you have selected is in the same sequence your currently have. Please pick different numbers or sequences.");
			}
		}
	}
	...
	/*public void addItem(int id, int quantity, int charges, boolean refresh) {
		addItem(id, quantity, charges, currentTab, refresh);
	}*/
	...
	public void collapse(int tabId) {
		...
		for (Item item : items)
			addItem(item.getId(), item.getAmount(), item.getCharges(), 0, false);
		refreshTabs();
		refreshItems();
	}
	
	private static final Map<Integer, Integer> TAB_INDEXES = new HashMap<Integer, Integer>();
	
	static {
		int index = 0;
		for (int i = 49; i >= 33; i-=2){
			TAB_INDEXES.put(i, index++);
		}
	}
	...
	public void switchItem(int fromSlot, int toSlot, int fromComponentId, int toComponentId) {
		...
		if (toSlot == 65535) {
			...
			//49 = 0, 47 = 1...
			//fuckin shit lol.. errhhh.
			
			int toTab = TAB_INDEXES.getOrDefault(toComponentId, -1);//toComponentId - 33;//8;//toComponentId >= 76 ? 8 - (84 - toComponentId) : 9 - ((toComponentId - 46) / 2);
			//33 = 8, 35 = 7...
			...
			if (bankTabs.length == toTab) {
				...
				if (!removed)
					...
					toTab -= 1;
				refreshTab(fromRealSlot[0]);
				addItem(item.getId(), item.getAmount(), item.getCharges(), toTab, true);
			}
		} else
	...
	public void openDepositBox() {
		...
		// tab
		sendBoxInterItems();
		player.getPackets().sendIComponentText(11, 12, "Bank Of " + Settings.SERVER_NAME + " - Deposit Box");
	...
	public void sendBoxInterItems() {
		player.getPackets().sendInterSetItemsOptionsScript(11, 16, 93, 7, 7, "Deposit-1", "Deposit-5", "Deposit-10",
				"Deposit-All", "Deposit-X", "Examine");
		player.getPackets().sendUnlockIComponentOptionSlots(11, 16, 0, 27, 0, 1, 2, 3, 4, 5);
	}
	...
	public void initBank() {		
		...
		refreshLastX();
		sendTabConfig();
		player.getPackets().sendAccessMask(0, 496, 762, 81, 40, 1278);	
		for (int i = 33; i <= 49; i += 2){
			player.getPackets().sendAccessMask(0, 1, 762, i, 40, 1278);
		}
        player.getPackets().sendAccessMask(0, 27, 763, 0, 36, 1150);
		refreshTabs();
		refreshItems();
	}
	...
	public void sendTabConfig() {
		int config = 0;
		config += getTabSize(2);
		config += getTabSize(3) * 1024;
		config += getTabSize(4) * 1048576;
		player.getPackets().sendConfig(1246, config);
		config = 0;
		config += getTabSize(5);
		config += getTabSize(6) * 1024;
		config += getTabSize(7) * 1048576;
		player.getPackets().sendConfig(1247, config);
		config = -2013265920;
		config += getTabSize(8);
		config += getTabSize(9) * 1024;
	...
	public void withdrawItem(int bankSlot, int quantity) {
		...
		if (item == null)
			return;
		if (item.getAmount() < quantity)
			item = new Item(item.getId(), item.getAmount(), item.getCharges());
		else
			item = new Item(item.getId(), quantity, item.getCharges());
		boolean noted = false;
		ItemDefinitions defs = item.getDefinitions();
		if (withdrawNotes) {
			if (!defs.isNoted() && defs.getCertId() != -1) {
				...
				player.getPackets().sendGameMessage("You cannot withdraw this item as a note.", true);
		}
		if (noted || defs.isStackable()) {
			if (player.getInventory().getItems().containsOne(item)) {
				int slot = player.getInventory().getItems().getThisItemSlot(item);
		...
		int numberOf = player.getInventory().getNumberOf(item.getId());
	...
	private void addItem(int id, int quantity, int charges, int creationTab, boolean refresh) {
		...
		if (slotInfo == null) {
			...
			tab[slot] = new Item(id, quantity, charges);
			...
			if (quantity + item.getAmount() < 0)
				return;
			bankTabs[slotInfo[0]][slotInfo[1]] = new Item(item.getId(), item.getAmount() + quantity, item.getCharges());
		}
		if (refresh)
			refreshItems();
	}
	...
	public void depositItem(int invSlot, int quantity, boolean refresh) {
		...
		if (amt < quantity)
			item = new Item(item.getId(), amt, item.getCharges());
		else
			item = new Item(item.getId(), quantity, item.getCharges());
		ItemDefinitions defs = item.getDefinitions();
		...
		if (bankedItem != null) {
			...
			player.getPackets().sendGameMessage("Not enough space in your bank.", true);
			return;
		}
		player.getInventory().deleteItem(invSlot, new Item(originalId, item.getAmount(), item.getCharges()));
		addItem(item, refresh);
	}

	private void addItem(Item item, boolean refresh) {
		addItem(item.getId(), item.getAmount(), item.getCharges(), refresh);
	}
	...
	private boolean removeItem(int[] slot, int quantity, boolean refresh, boolean forceDestroy) {
		...
		if (quantity >= item.getAmount()) {
			...
			bankTabs[slot[0]][slot[1]] = new Item(item.getId(), item.getAmount() - quantity, item.getCharges());
		if (refresh)
			refreshItems();
		return destroyed;
	}
	...
	public void sendItems() {
		player.getPackets().sendItems(-1, 0, 95, getContainerCopy());
	}

	public void refreshItems(int[] slots) {
		player.getPackets().sendUpdateItems(-1, 0, 95, getContainerCopy(), slots);
	}
	...
	public void refreshItems() {
		...
		player.getPackets().sendConfig(192, getBankSize() - 1);
		//long bankValue = player.getBank().getBankValue();
		player.getPackets().sendIComponentText(762, 32, "Bank of "+Settings.SERVER_NAME+"."); //<col=00ff00>(Bank Wealth: "+ Utils.formatDoubledAmount(bankValue)+ ")");
	}
	...
	public void switchInsertItems() {
		...
		player.getPackets().sendConfigByFile(305, insertItems ? 1 : 0);
	}
