...
import com.rs.game.npc.others.MetalArms;
...
public final class Metals extends Controler {
	
	private MetalArms target;
	...
	public boolean canAttack(Entity target) {
		if (target instanceof MetalArms && target != this.target) {
	...
	private void leave(boolean logout) {
		...
		if(!logout) {
			player.getPackets().blackout(0); //unblacks minimap
	...
	public void sendTarget(int id, WorldTile tile) {
		if(target != null) 
			target.disapear();
		target = new MetalArms(id, tile, this);
	...
	public void process() {
		...
		if(headComponentId == 0) {
			...
			if (activeLevel > 0) {
				...
				player.getSkills().drainPrayer(level / 6);
			}
			timer = 3;
		}else{
	...
	public void sendInterfaces() {
		if(player.getHiddenBrother() != -1)
			player.getInterfaceManager().sendTab(player.getInterfaceManager().hasRezizableScreen() ? 24 : 0, 24);
	}
	...
	public void loadData() {
		...
		player.getPackets().blackout(2); //blacks minimap
	}
	
	public void sendBrotherSlain(int index, boolean slain) {
		player.getPackets().sendConfig(457+index, slain ? 1 : 0);
	}
	
	public void sendCreaturesSlainCount(int count) {
		player.getPackets().sendConfig(464, count);
	}
