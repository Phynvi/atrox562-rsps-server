...
public class FightCaves extends Controler {
	...
	private static final WorldTile OUTSIDE = new WorldTile(2438, 5168, 0);
	...
	private void loadCave(final boolean login) {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				RegionBuilder.copyAllPlanesMap(296, 632, boundChuncks[0], boundChuncks[1], 8);
				// selects a music
	...
	private void startWave() {
		...
		player.getPackets().sendGameMessage("<col=db0000>Wave "+ currentWave);
		if (stage != Stages.RUNNING)
			return;
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		exitCave(1);
		//player.getDialogueManager().startDialogue("SimpleMessage", "You can't just leave like that.");
		return false;
	}
		...
		if (type == 0 || type == 2)
			...
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					...
					// player.jadCount++;
					// player.getPackets().sendGameMessage("<col=1C9800>Your
					// Tztok-Jad killcount is:
					// </col><col=ff0000>"+(player.jadCount)+".");
					World.sendWorldMessage(
							"<img=6><col=ff8c38>News: " + Utils.formatPlayerNameForDisplay(player.getUsername())
									+ " has just completed the Fight Caves!",
							false);
