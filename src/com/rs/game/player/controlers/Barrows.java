...
public final class Barrows extends Controler {
	...
	private void leave(boolean logout) {
		...
		if (!logout) {
			for (int varBit : TUNNEL_CONFIG[getTunnelIndex()])
				player.getVarBitManager().sendVar(varBit, 0);
	...
	public boolean processObjectClick1(WorldObject object) {
		if (object.getId() >= 6702 && object.getId() <= 6707) {
			...
			player.lock(3);
			player.getVarBitManager().sendVar(1270, inBetween ? 0 : 1);
			if (player.getHiddenBrother() != -1) {
	...
	public void process() {
		...
		if (headComponentId == 0) {
			if (player.getHiddenBrother() == -1) {
				player.applyHit(new Hit(player, Utils.random(5) + 1, HitLook.REGULAR_DAMAGE));
				resetHeadTimer();
				return;
			}
	...
	public void loadData() {
		...
		for (int varBit : TUNNEL_CONFIG[getTunnelIndex()])
			player.getVarBitManager().sendVar(varBit, 1);
		player.getVarBitManager().sendVar(467, 1);
	}
	...
	public void sendCreaturesSlainCount(int count) {
		player.getPackets().sendConfigByFile(464, player.getBarrowsKillCount());
	}
