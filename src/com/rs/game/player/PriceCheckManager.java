...
public class PriceCheckManager {
	...
	final private static int tradeLimit = 120000;
	...
	public void openPriceCheck() {
		player.getPackets().sendButtonConfig(728, 0);
		player.getPackets().sendButtonConfig(729, tradeLimit);
		player.getInterfaceManager().sendInterface(206);
		player.getInterfaceManager().sendInventoryInterface(207);
		sendInterItems();
		sendOptions();
		player.getPackets().sendConfig(728, 0);
		for (int i = 0; i < pcInv.getSize(); i++)
			player.getPackets().sendConfig(700 + i, 0);
	...
	public void refreshItems(Item[] itemsBefore) {
		...
		for (int index = 0; index < itemsBefore.length; index++) {
			...
			if (item != null)
				totalPrice += item.getDefinitions().getTipitPrice() * item.getAmount();
			if (itemsBefore[index] != item) {
				changedSlots[count++] = index;
				player.getPackets().sendButtonConfig(700 + index,
						item == null ? 0 : item.getDefinitions().getTipitPrice());
			}
		...
		player.getPackets().sendButtonConfig(728, totalPrice);
		player.getPackets().sendButtonConfig(729, tradeLimit);
	}

	public void refresh(int... slots) {
		player.getPackets().sendUpdateItems(-1, 0, 90, pcInv, slots);
	}
	...
	public void sendInterItems() {
		player.getPackets().sendItems(-1, 0, 90, pcInv);
	}
