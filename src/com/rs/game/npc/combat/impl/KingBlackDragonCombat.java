	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (attackStyle == 0) {
			...
			final Player player = target instanceof Player ? (Player) target
					: null;
			if (Combat.hasAntiDragProtection(target)) {
				damage = getRandomMaxHit(npc, 17, NPCCombatDefinitions.MAGE,
						target);
				if (player != null)
					player.getPackets()
							.sendGameMessage(
				...
				damage = getRandomMaxHit(npc, 18, NPCCombatDefinitions.MAGE,
						target);
				if (player != null)
					player.getPackets()
							.sendGameMessage(
			...
			if (Utils.getRandom(2) == 0)
				target.getPoison().makePoisoned(8);
			delayHit(npc, 2, target, getRegularHit(npc, damage));
