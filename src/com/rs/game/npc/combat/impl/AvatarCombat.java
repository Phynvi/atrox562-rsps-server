	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (attackStyle == 0) {
			...
			if (player != null
				...
				damage = 0;
			if (player != null
					&& (player.getFireImmune() > Utils.currentTimeMillis() || player.getFireImmune() > Utils.currentTimeMillis())) {
				if (damage != 0)
					damage = Utils.getRandom(164);
			} else if (damage == 0)
				damage = Utils.getRandom(164);
			else if (player != null)
				player.getPackets().sendGameMessage("You are hit by the avatar's powerful smash!", true);
			...
			npc.setNextForceTalk(new ForceTalk("Lemme Smash!"));
			...
			if (player != null
				...
				damage = getRandomMaxHit(npc, 164, NPCCombatDefinitions.MAGE,
						target);
				if (player != null)
					player.getPackets().sendGameMessage("You are hit by the avatar's powerful smash!",
							true);
			} else {
