...
public abstract class Familiar extends NPC implements Serializable {
	...
	public void processNPC() {
		...
		if (trackTimer == 50) {
			...
			if (ticks == 2)
				...
				owner.getInterfaceManager().closeSummoning();
		...
		if (Wilderness.isAtWild(this)) {
			if (NPCDefinitions.getNPCDefinitions(getId() + 1).getName().equals(getName()))
	...
	public boolean canAttack(Entity target) {
		...
		if (target instanceof Player) {
			...
			if (!owner.isCanPvp() || !player.isCanPvp() || owner == player)
				return false;
		}
	...
	public boolean renewFamiliar() {
		if (ticks > 5) {
			owner.getPackets().sendGameMessage(
					"You need to have at least two minutes and fifty seconds remaining before you can", true);
			owner.getPackets().sendGameMessage("renew your familiar.", true);
			return false;
	...
	public void sendFollowerDetails() {
		...
		owner.getPackets().sendInterface(true, res ? 746 : 548, res ? 37 : 92, 662);
		...
		unlock();
		owner.getPackets().sendButtonConfig(168, 8);// tab id
	}
	...
	public static void confirmLeftOption(Player player) {
		...
		player.getPackets().sendInterface(true, res ? 746 : 548, res ? 37 : 92, 662);
		player.getPackets().sendHideIComponent(747, 8, false);
	}

	public static void selectLeftOption(Player player) {
		boolean res = player.getInterfaceManager().hasRezizableScreen();
		player.getPackets().sendInterface(true, res ? 746 : 548, res ? 37 : 92, 880);
		sendLeftClickOption(player);
		player.getPackets().sendButtonConfig(168, 8);// tab id
	}
	...
	public void call() {
		if (getAttackedBy() != null && getAttackedByDelay() > Utils.currentTimeMillis()) {
			owner.getPackets().sendGameMessage("You cant call your familiar whilst it's under combat.");
			return;
		}
		call(false);
	}

	public void chopTree(WorldObject object) {
		if (World.canMoveNPC(object.getPlane(), object.getX(), object.getY(), 1))

			setNextAnimation(new Animation(7722));
	}
	...
	public void dissmissFamiliar(boolean logged) {
		...
		if (!logged && !isFinished()) {
			setFinished(true);
			switchOrb(false);
			owner.getPackets().sendButtonConfig(168, 4);
			owner.getPackets().sendIComponentSettings(747, 18, 0, 0, 0);
			if (bob != null)
				bob.dropBob();
		}
