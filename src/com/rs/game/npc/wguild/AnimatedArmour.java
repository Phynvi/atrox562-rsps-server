...
public class AnimatedArmour extends NPC {
	...
	public AnimatedArmour(Player player, int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
			...
			public void run() {
				if (loop == 0) {
					...
					if (source instanceof Player) {
						...
						World.addGroundItem(new Item(8851, WarriorsGuild.ARMOR_POINTS[getId() - 4278]), new WorldTile(getCoordFaceX(getSize()), getCoordFaceY(getSize()), getPlane()), player, true, 60);
					}
					finish();
					stop();
				}
