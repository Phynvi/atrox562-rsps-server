...
public class GraveStone extends NPC {// 652 - gravestone selection interface
	...
	public GraveStone(Player player, WorldTile deathTile, Item[] items) {
		...
		player.getPackets().sendConfig(623, deathTile.getTileHash());
		player.getPackets().sendConfig(624, 0);
	...
	public void finish() {
		...
		if (player != null) {
			...
			player.getPackets().sendConfig(623, -1);
	...
	public void sendGraveInscription(Player reader) {
		...
		//reader.getVarBitManager().sendVar(4191, graveStone == 0 ? 0 : 1);
	...
	public static Integer[][] getItemSlotsKeptOnDeath(final Player player, boolean atWilderness, boolean skulled, boolean protectPrayer) {
		...
		for (int i = 1; i < 44; i++) {
			...
			if (item == null)
				continue;
			int stageOnDeath = -1;/*item.getDefinitions().getStageOnDeath();*/
		...
		Collections.sort(droppedItems, new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				...
				int price1 = i1 == null ? 0 : i1.getDefinitions().getTipitPrice();
				int price2 = i2 == null ? 0 : i2.getDefinitions().getTipitPrice();
				if (price1 > price2)
					return -1;
				if (price1 < price2)
					return 1;
