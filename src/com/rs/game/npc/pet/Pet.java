...
public final class Pet extends NPC {
	...
	public void processNPC() {
		unlockOrb();
		if (pet.getFood().length > 0) {
			details.updateHunger(0.025);
			// owner.getPackets().sendConfig(4286, (int) details.getHunger());
		}
		if (details.getHunger() >= 90.0 && details.getHunger() < 90.025) {
			...
			switchOrb(false);
			// owner.getPackets().closeInterface(owner.getInterfaceManager().hasRezizableScreen()
			// ? 98 : 212);
		...
		if (growthRate > 0.000) {
			...
			// owner.getPackets().sendConfig(4285, (int) details.getGrowth());
	...
	public void sendMainConfigurations() {
		...
		owner.getPackets().sendConfig(1436, 0);
		unlockOrb(); // temporary
	}
	...
	public void sendFollowerDetails() {
		...
		// owner.getPackets().sendInterface(true, res ? 746 : 548, res ? 119 :
		// 179, 662);
		unlock();
		owner.getPackets().sendConfig(168, 8);// tab id
	}
