...
public class DuelArena extends Controler {
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		...
		player.getPackets().sendItems(-1, 0, 134, player.getLastDuelRules().getStake());
		player.getPackets().sendItems(-2, 0, 134, player.getLastDuelRules().getStake());
		player.getPackets().sendIComponentText(ifFriendly ? 637 : 631, ifFriendly ? 16 : 25,
				" " + Utils.formatPlayerNameForDisplay(target.getUsername()));
		player.getPackets().sendIComponentText(ifFriendly ? 637 : 631, ifFriendly ? 18 : 27,
	...
	private void refresh(int... slots) {
		player.getPackets().sendUpdateItems(-1, 0, 134, player.getLastDuelRules().getStake(), slots);
		target.getPackets().sendUpdateItems(-2, 0, 134, player.getLastDuelRules().getStake().getItems(), slots);
	}
	...
	private void refreshScreenMessage(boolean firstStage, boolean ifFriendly) {
		player.getPackets().sendIComponentText(firstStage ? (ifFriendly ? 637 : 631) : (ifFriendly ? 639 : 626),
				firstStage ? (ifFriendly ? 45 : 28) : (ifFriendly ? 23 : 45),
	...
	public void endDuel(final Player victor, final Player loser, boolean removeLoserControler) {
		...
		loser.getInterfaceManager().sendInterface(634);
		victor.getPackets().sendItems(-1, 28, 136, loser.getLastDuelRules().getStake());
		loser.getPackets().sendItems(-1, 28, 136, loser.getLastDuelRules().getStake());
		victor.getPackets().sendIComponentText(634, 23, ""+ loser.getDisplayName() +"");
		victor.getPackets().sendIComponentText(634, 22, ""+ loser.getSkills().getCombatLevelWithSummoning() +"");
		loser.getPackets().sendIComponentText(634, 23, ""+ victor.getDisplayName() +"");
		loser.getPackets().sendIComponentText(634, 22, ""+ victor.getSkills().getCombatLevelWithSummoning() +"");
		loser.getPackets().sendIComponentText(634, 15, "You have been Defeated!");
	...
	private void beginBattle(boolean started) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count > 0)
					player.setNextPublicChatMessage(new PublicChatMessage("" + count, 0));
				if (count == 0) {
					player.getTemporaryAttributtes().put("canFight", true);
					player.setNextPublicChatMessage(new PublicChatMessage("FIGHT!", 0));
					this.stop();
				}
				count--;
			}
		...
		synchronized (this) {
			synchronized (target.getControlerManager().getControler()) {
				...
				case 631:
					...
					case 29: // no range
					case 30:
						if (rules.getRule(1) && rules.getRule(2)) {
							player.sendMessage("You have to be able to use at least one combat style in a duel.");
							return false;
						}
						rules.setRules(0);
						return false;
					case 31: // no melee
					case 32:
						if (rules.getRule(0) && rules.getRule(2)) {
							player.sendMessage("You have to be able to use at least one combat style in a duel.");
							return false;
						}
						rules.setRules(1);
					...
					case 33: // no magic
					case 34:
						if (rules.getRule(0) && rules.getRule(1)) {
							player.sendMessage("You have to be able to use at least one combat style in a duel.");
							return false;
						}
						rules.setRules(2);
