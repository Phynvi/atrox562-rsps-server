...
public final class CastleWars {
	...
	public static void joinPortal(Player player, int team) {
		...
		player.setNextWorldTile(new WorldTile(team == ZAMORAK ? ZAMO_WAITING
				: SARA_WAITING, 1));
		player.getMusicsManager().playMusic(318); // testing 2 players
	...
	public static void createPlayingGame() {
		...
		CoresManager.fastExecutor1.scheduleAtFixedRate(playingGame, 60000, 60000);
	...
	public static void refreshTimeLeft(Player player) {
		player.getPackets().sendConfig(380, playingGame == null ? 0 : playingGame.minutesLeft - (player.getControlerManager().getControler() instanceof CastleWarsPlaying ? 5 : 0));
	}
	...
	public static class PlayingGame extends TimerTask {
		...
		public void refresh(Player player) {
			player.getPackets().sendConfig(143, flagStatus[SARADOMIN]);
			player.getPackets().sendConfig(145, score[SARADOMIN]);
			player.getPackets().sendConfig(153, flagStatus[ZAMORAK]);
			player.getPackets().sendConfig(155, score[ZAMORAK]);
		}
