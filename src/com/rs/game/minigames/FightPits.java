...
public final class FightPits {
	...
	private static WorldTile[] GAME_TELEPORTS = {
			new WorldTile(2412, 5153, 0),
			new WorldTile(2406, 5138, 0),
			new WorldTile(2394, 5139, 0),
			new WorldTile(2382, 5153, 0),
			new WorldTile(2398, 5156, 0),
			new WorldTile(2404, 5145, 0)};
						...
						if(minutes == 0) {
							...
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									...
									for(Player player : arena)  
										player.applyHit(new Hit(player, 15, HitLook.REGULAR_DAMAGE));
								}
	...
	public static void leaveArena(Player player, int type) {
		synchronized (lock) {
			...
			if(type == 0) {
				player.setLocation(2404, 5180, 0);
			}else {
				if(type != 3)
					lobby.add(player);
				...
				if(player.hasSkull() && player.getSkullId() == 1) {//if has champion skull
					...
					if(!player.getInventory().addItem(6529, tokkul) && type == 1) 
						World.addGroundItem(new Item(6529, tokkul), new WorldTile(2399, 5178, 0), player, true, 180, true);
				}
				if(type == 1) {
					player.lock(5);
					player.addWalkSteps(2399, 5170, 0, false);
				}else if(type == 2)
					player.setNextWorldTile(new WorldTile(new WorldTile(2399, 5178, 0), 2));
			}
			refreshFoes();
			checkPlayersAmmount();
			if(startedGame && arena.size() <= 1)//<= 1)
				endGame();
		}
	}
	...
	public static void enterLobby(Player player, boolean login) {
		synchronized (lock) {
			if(!login) {
				player.lock(5);
				player.addWalkSteps(2399, 5174, 0, false);
				player.getControlerManager().startControler("FightPitsLobby");
			}
			lobby.add(player);
			checkPlayersAmmount();
	...
	public static void leaveLobby(Player player, int type) {
		synchronized (lock) {
			if(type != 0) {
				if(type == 1) {
					player.lock(5);
					player.addWalkSteps(2399, 5177, 0, false);
				}
				player.getControlerManager().removeControlerWithoutCheck();
			}
			lobby.remove(player);
	...
	public static void startGame(boolean end) {
		...
		CoresManager.fastExecutor.scheduleAtFixedRate(gameTask, end ? 60: 70, 60, TimeUnit.SECONDS);
	...
	public static void setChampion() {
		...
		champion.getPackets().sendIComponentText(373, 1, "Current Champion: JaLYt-Ket-" + currentChampion);
