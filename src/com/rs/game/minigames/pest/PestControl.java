...
public class PestControl {
	...
	private final static int[][] PORTAL_LOCATIONS = { { 4, 56, 45, 21, 32 },
			{ 31, 28, 10, 9, 32 } };
	private final static int[][] SPAWN_LOCATIONS = { { 9, 54, 46, 22, 1 },
			{ 32, 30, 14, 13, 1 } };
	...
	public PestControl create() {
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				CoresManager.fastExecutor1.schedule(new PestGameTimer(), 1000,
						1000);
			}
		});
		return instance;
	...
	private void sendBeginningWave() {
		...
		for (int index = 0; index < portals.length; index++) {
			...
			portal.setHitpoints(data.ordinal() == 0 ? 200 : 250);
		}
	}
	...
	public boolean createPestNPC(int index) {
		if (pestCounts[index] >= (index == 4 ? 4
			...
			return false;
		pestCounts[index]++;
		WorldTile baseTile = getWorldTile(SPAWN_LOCATIONS[0][index],
				SPAWN_LOCATIONS[1][index]);
	...
	private void sendFinalReward(Player player, int knightZeal) {
		if (knight.isDead())
			...
			player.getDialogueManager()
					.startDialogue(
							"SimpleMessage",
							"Congratulations! You have successfully kept the lander safe and have been awarded: "
									+ coinsAmount
									+ " gold coins and "
	...
	private void sendPortalInterfaces() {
		for (Player player : team) {
			...
			player.getPackets().sendIComponentText(408, 1, "" + knight.getHitpoints() / 10);
		}
	}
	...
	public enum PestData {
		...
		3742, 3743, 3744, /* Brawler */3772, 3773, /* Splatter */3727, 3728, 3729, /* Spinner */
				...
				3762, 3763, 3764, 3765 }, new int[] { 3732, 3733, 3734, 3735 },
				5),
		...
		VETERAN(new int[] { /* Shifters */3736, 3737, 3738, 3739, 3740,
				...
				3736, 3737, 3738, 3739, 3740, 3741 }, 15);
