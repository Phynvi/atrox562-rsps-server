...
public final class BrimhavenAgility extends Controler {
	...
	private static void removePlayer(Player player) {
		...
		if(player.getTemporaryAttributtes().remove("BrimhavenAgility") != null)
			player.getPackets().sendConfig(4456, 0);
	...
	private static void startGame() {
		...
		CoresManager.fastExecutor1.scheduleAtFixedRate(currentGame = new PlayingGame(), 0, 60000);
		CoresManager.fastExecutor1.scheduleAtFixedRate(bladesManager = new BladesManager(), 5000, 5000); //TODO right time atm they move each 5seconds
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if(object.getId() == 3581 || object.getId() == 3608) {
			...
			if(stage == null) {
				...
				player.getPackets().sendConfig(4456, 1); //ready to get tickets
