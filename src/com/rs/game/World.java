...
public final class World {
	...
	private static HashMap<String, Clan> cachedClans = new HashMap<String, Clan>();
    ...
    public static final void init() {
		...
		NPCForceTalks.forceTalkTask();
    	addRestoreSpecialAttackTask();
		PyramidPlunderControler.PyramidPlunderTask();
	...
	public static void addCachedClan(String clanName) {
		cachedClans.put(clanName.toLowerCase().replaceAll("_", " "), SerializableFilesManager.loadClan(clanName));
	}

	public static void removeCachedClan(String clanName) {
		cachedClans.remove(clanName);
	}

	public static Clan getClan(String clanName) {
		return cachedClans.get(clanName.toLowerCase().replaceAll("_", " "));
	}

	public static final void initiateClans() {
		...
		for (File file : files) {
			if (file.getName().endsWith(".c")) {
				if (SerializableFilesManager.containsClan(file.getName().replace(".c", "")))
					cachedClans.put(file.getName().replace(".c", "").toLowerCase(),
							SerializableFilesManager.loadClan(file.getName().replace(".c", "")));
				else
					Logger.log("Tried loading a clan that didn't exist.");
			}
		}
	}
	
	public static final WorldObject getObjectWithType(WorldTile tile, int type) {
		return getRegion(tile.getRegionId(), true).getObjectWithType(tile.getPlane(), tile.getXInRegion(),
    ...
    private static final void addRestoreHitPointsTask() {
    	CoresManager.fastExecutor.scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
					for(Player player : players) {
						if(player == null || player.isDead() || !player.isRunning() || !player.acceptAid)
							continue;
				...
				try {
					for (Player player : players) {
						if (player == null || !player.isRunning())
							continue;
						int amountTimes = player.getPrayer().usingPrayer(0, 8) ? 2 : 1;
						if (player.isResting())
							amountTimes += 1;
	...
	public static final NPC spawnNPC(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea, String direction) {
		NPC returnValue = spawnNPC(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, false);
		WorldTile directionalTile = tile;
		direction = direction.toLowerCase();
			
		if(direction.contains("north"))
			directionalTile.setLocation(tile.getX(), tile.getY() + 1, tile.getPlane());
		else if(direction.contains("south"))
			directionalTile.setLocation(tile.getX(), tile.getY() - 1, tile.getPlane());
			
		if(direction.contains("east"))
			directionalTile.setLocation(tile.getX() + 1, tile.getY(), tile.getPlane());
		else if(direction.contains("west"))
			directionalTile.setLocation(tile.getX() - 1, tile.getY(), tile.getPlane());

		returnValue.setNextFaceWorldTile(directionalTile);
		return returnValue;
	}
	...
	public static final NPC spawnNPC(int id, WorldTile tile,
			...
			boolean spawned) {
		NPC n = null;
		Birds birds = Birds.forId(id);
		if (birds != null) {
			if (id == birds.getNpcId())
				return new BirdSnaringNPCs(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		}
		BoxEntities boxNPCs = BoxEntities.forId(id);
		if (boxNPCs != null) {
			if (id == boxNPCs.getNpcId())
				return new BoxHunterNPCs(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		}
		DeadfallPreys deadfall = DeadfallPreys.forId(id);
		if (deadfall != null) {
			if (id == deadfall.getNpcId())
				return new Kebbits(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		}
		NetPreys net = NetPreys.forId(id);
		if (net != null) {
			if (id == net.getNpcId())
				return new NetTrapNPCs(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		}
		PitfallPreys pitfall = PitfallPreys.forId(id);
		if (pitfall != null) {
			if (id == pitfall.getNpcId())
				return new PitfallNPCs(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
	...
	public static final void updateEntityRegion2(Entity entity) {
		if(entity.hasFinished()) {
			if(entity instanceof Player)
				getRegion(entity.getLastRegionId()).removePlayerIndex(entity.getIndex());
			else
		...
		if(entity.getLastRegionId() != regionId) { //map region entity at changed
			...
			if(entity instanceof Player) {
				Player player = (Player) entity;
				player.getControlerManager().moved();
				if(player.getMusicsManager().musicEnded())
	...
	public static final void updateGroundItem1(Item item, final WorldTile tile, final Player owner) {
	...
	public static final boolean removeGroundItem(FloorItem floorItem) {
	...
	public static final boolean removeGroundItem(Player player, FloorItem floorItem) {
		...
		region.forceGetFloorItems().remove(floorItem);
		player.getInventory().addItem(floorItem.getId(), floorItem.getAmount());
		if (floorItem.isInvisible() || floorItem.isGrave()) { //|| floorItem.isForever()) {
			player.getPackets().sendRemoveGroundItem(floorItem);
			return true;
		} else {
			for (Player p2 : World.getPlayers()) {
	...
	public static final boolean isMultiArea(WorldTile tile) {
		...
		return (destX >= 3462 && destX <= 3511 && destY >= 9481 && destY <= 9521 && tile.getPlane() == 0) // kalphite
				...
				|| (destX >= 3137 && destX <= 3173 && destY >= 4225 && destY <= 4281) // cockroach area
				|| (destX >= 3786 && destX <= 3824 && destY >= 3530 && destY <= 3567) // donator boss
				|| (destX >= 2898 && destX <= 2930 && destY >= 4433 && destY <= 4467) // dagannoth kings
				// godwars
	...
	public static final boolean atMultiArea(Player player) {
		return (AreaManager.get(player) != null && (AreaManager.get(player)
				.name().equalsIgnoreCase("Multi Area")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("Wildy Agility Multi")
				|| AreaManager.get(player).name().equalsIgnoreCase("Godwars")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("ForinthryMulti")
				|| AreaManager.get(player).name().equalsIgnoreCase("Nex Multi")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("Corp Multi")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("Al Kharid")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("Tormdemon Multi")
				|| AreaManager.get(player).name().equalsIgnoreCase("KQ Multi")
				|| AreaManager.get(player).name()
						.equalsIgnoreCase("Chaos Tunnel Multi")
				|| AreaManager.get(player).name().equalsIgnoreCase("Dks Multi") || AreaManager
				.get(player).name().equalsIgnoreCase("KBD Multi")));
	...
	public static final void addPermanentGroundItem(final Item item, final WorldTile tile, final int tick,
	...
	/*public static final void addGroundItem1(final Item item, final WorldTile tile) {
	...
	private static final void addGroundItem(final Item item, final WorldTile tile,
			final Player owner/* null for default */, final boolean underGrave,
	...
	public static void sendWorldWideMessage(String s){
		for (Player p: getPlayers()){
			...
			p.getPackets().sendGameMessage(s);
		}
	}
	...
	public static final void sendElementalProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
			if (playersIndexes == null)
			...
			for (Integer playerIndex : playersIndexes) {
				...
				player.getPackets()
						.sendProjectile(
								...
								43,
								34,
								distance < 2 ? 51 : distance == 2 ? 56 : 61,
								51,
								6,
								Utils.getDistance(shooter, receiver) > 3 ? 0
										: 11, size);
			}
		}
	}

	public static final void sendFastBowProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
			...
			for (Integer playerIndex : playersIndexes) {
				...
				player.getPackets().sendProjectile(
						...
						41,
						6,
						receiver instanceof Player ? startOffsetDistance
								: startOffsetDistance2, size);
			}
		}
	}

	public static final void sendMSBProjectile(Entity shooter, Entity receiver,
			int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendMSBProjectile2(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendFastBowSwiftProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendSlowBowProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
			...
			for (Integer playerIndex : playersIndexes) {
				...
				player.getPackets().sendProjectile(
						...
						34,
					//	30,//speed
						distance < 2 ? 30 : distance == 2 ? 34 : 36,
						//distance < 2 ? 41 : distance == 2 ? 46 : 51,
						41,
						16,
						receiver instanceof Player ? startOffsetDistance
								: startOffsetDistance2, size);
	...
	public static final void sendSlowBow2Projectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendSlowBowProjectile1(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
			...
			for (Integer playerIndex : playersIndexes) {
				...
				int startOffsetDistance = distance > 2 ? 0 : 11;
				int startOffsetDistance2 = 0;
				int speed = (int) (55 + (shooter.getWorldTile().getDistance(receiver.getWorldTile()) * 10));
				player.getPackets().sendProjectile(
						receiver,
						new WorldTile(shooter.getCoordFaceX(size), shooter
								.getCoordFaceY(size), shooter.getPlane()),
	...
	public static final void sendSlowBow2Projectile1(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendSlowBowSwiftProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendCBOWProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
			...
			for (Integer playerIndex : playersIndexes) {
				...
				if (player == null
						...
						|| (!player.withinDistance(shooter) && !player
								.withinDistance(receiver)))
					continue;
				int size = shooter.getSize()<< 5;
				int distance = Utils.getDistance(shooter, receiver);
				int startOffsetDistance = distance > 2 ? 0 : 11;
				int startOffsetDistance2 = 0;
				player.getPackets().sendProjectile(
						receiver,
						new WorldTile(shooter.getX(), shooter.getY(), shooter
								.getPlane()),
						receiver,
						gfxId,
						43,
	...
	public static final void sendCBOWSwiftProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendThrowSwiftProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendThrowProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendCannonProjectile(Entity shooter,
			Entity receiver, int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void sendSOAProjectile(Entity shooter, Entity receiver,
			int gfxId) {
		for (int regionId : shooter.getMapRegionsIds()) {
			List<Integer> playersIndexes = getRegion(regionId)
					.getPlayerIndexes();
	...
	public static final void spawnObjectTemporary(final Player player,
			final WorldObject object, long time) {
	...
	public static final boolean removeObjectTemporary(final Player player,
			final WorldObject object, long time, boolean removeClip) {
	...
	public static final void spawnTempGroundObject(final WorldObject object,
			final int replaceId, long time, final boolean removeClip) {
		spawnObject(object);
		CoresManager.slowExecutor.schedule(new Runnable() {
			@Override
			public void run() {
	...
	public static final boolean isSpawnedObject(Player player,
			WorldObject object) {
		return getRegion(player.getRegionId()).getSpawnedObjects().contains(
				object);
	}
	...
	public static final void spawnObject(Player player, WorldObject object) {
		getRegion(player.getRegionId()).spawnObject(object,
				object.getPlane(), object.getXInRegion(),
	...
	public static final void removeObject(Player player, WorldObject object,
			boolean removeClip) {
		getRegion(player.getRegionId()).removeObject(object,
				object.getPlane(), object.getXInRegion(),
				object.getYInRegion());
	}
	...
	public static final void removeObject(Player player, WorldObject object) {
		getRegion(player.getRegionId())
				.removeObject(object, object.getPlane(),
						object.getXInRegion(), object.getYInRegion());
	}
	...
	public static final void addGroundItem(final Item item,
			final WorldTile tile, int publicTime) {
	...
	public static final FloorItem addGroundItem(final Item item,
		...
		final FloorItem floorItem = new FloorItem(item, tile, owner, false,
				invisible);
		final Region region = getRegion(tile.getRegionId());
		if (type == 1) {
			if (ItemConstants.isTradeable(item)
	...
	/*public static final void updateGroundItem(Item item, final WorldTile tile,
		 ...
		 * floorItem.getDefinitions().getName() + " set from " +
		 * floorItem.getAmount() + " to " + (floorItem.getAmount() +
		 * item.getAmount()) + "");
		 */
		//floorItem.setAmount(floorItem.getAmount() + item.getAmount());
		/*
	...
	public static final void updateGroundItem(Item item, final WorldTile tile,
			final Player owner, final int time, final int type) {
		final FloorItem floorItem = World.getRegion(tile.getRegionId())
				.getGroundItem(item.getId(), tile, owner);
		if (floorItem == null) {
